...
grep -q 'VPS Ubuntu' <<<"$help_output"
grep -q 'Node.js 20' <<<"$help_output"
grep -q 'firewall do provedor' <<<"$help_output"
...
grep -Fq 'CONFIG_FILE="$CONFIG_FILE" APP_DIR="$APP_DIR" PNPM_CLI="$PNPM_CLI"' "$INSTALLER"
grep -Fq 'DATABASE_URL="$MIGRATION_DATABASE_URL" node "$PNPM_CLI" exec drizzle-kit migrate' "$INSTALLER"
...
grep -q 'CALLER="${SUDO_USER:-${USER:-root}}"' "$INSTALLER"
grep -q 'if \[\[ "$CALLER" == "root" \]\]; then' "$INSTALLER"
grep -q 'git clone --depth=1 "$REPOSITORY_URL" "$CLONE_DIR/project"' "$INSTALLER"
